#!/usr/bin/env python3
"""Recompute the matched-S2 residual-stream intervention used in the paper.
...
def load_model(
    ...
    patch_layers = [int(layer) for layer in entry["layers"]]
    resolved_device = device or ("cuda" if torch.cuda.is_available() else "cpu")

    if resolved_device == "cpu" and model_key in {
        "pythia-2.8b",
        "pythia-6.9b",
        "pythia-12b",
    }:
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for start in range(0, len(records), args.batch_size):
        ...
        controls = [record.control_prompt for record in batch_records]
        ...
        with torch.inference_mode(), autocast_context():
            control_cache = model.run_with_cache(
                ...
                in {f"blocks.{layer}.hook_resid_post" for layer in patch_layers},
            )[1]
            ...
            def patch_hook(resid: torch.Tensor, hook: Any) -> torch.Tensor:
                ...
                patched[row_index, position_index, :] = donor[
                    row_index,
                    position_index,
                    :,
                ]
        ...
        for row, record in enumerate(batch_records):
            ...
            changed = float(
                (patched_logits[row, io_id] - patched_logits[row, s_id]).item()
            )
    ...
    result: dict[str, Any] = {
        ...
        "n_templates": 10,
        "prompts_per_template": 30,
        "template_family": "ten BABA-style IOI templates",
        ...
        "delta_ld_ci_prompt": bootstrap_mean(
            delta,
