#!/usr/bin/env python3
"""Regenerate the three camera-ready figures from committed JSON artifacts."""
...
def figure1(out_dir: Path) -> None:
    ...
    for label, values in series.items():
        ...
        accuracy = 100 * np.asarray([float(v["ioi_acc"]) for _, v in values])
    ...
    ax.set(xscale="log", ylim=(0, 105), xlabel="Training step", ylabel="IOI accuracy (%)",
...
def figure2(out_dir: Path) -> None:
    trajectory = rows(load("signflip_across_scale_160m_410m_1b.json")["pythia-160m"]["by_step"])
    suppressor = rows(load("suppressor_ablation_trajectory.json")["pythia-160m"]["by_step"])
    step = np.asarray([x for x, _ in trajectory], dtype=float)
    accuracy = 100 * np.asarray([float(v["ioi_acc"]) for _, v in trajectory])
    effect = np.asarray([float(v["delta_ld_mean"]) for _, v in trajectory])
    effect_lo = np.asarray([float(v["delta_ld_ci"][0]) for _, v in trajectory])
    effect_hi = np.asarray([float(v["delta_ld_ci"][1]) for _, v in trajectory])
    sstep = np.asarray([x for x, _ in suppressor], dtype=float)
    seffect = np.asarray([float(v["ablation_delta"]) for _, v in suppressor])
    slo = np.asarray([float(v["ablation_ci"][0]) for _, v in suppressor])
    shi = np.asarray([float(v["ablation_ci"][1]) for _, v in suppressor])
    ...
    axes[0].annotate("floor", (2000, 31.7), xytext=(0, -15), textcoords="offset points", ha="center", fontsize=7)
    axes[0].annotate(">chance", (3000, 57.3), xytext=(0, 7), textcoords="offset points", ha="center", fontsize=7)
    ...
    axes[2].plot(sstep, seffect, marker="o", linewidth=1.4, markersize=3)
    axes[2].fill_between(sstep, slo, shi, alpha=0.18)
    axes[2].axhline(0, linewidth=0.8, color="0.45")
    axes[2].set(ylabel="Suppressor mean-ablation effect", title="Mean-ablate suppressor")
...
def figure3(out_dir: Path) -> None:
    ...
    trajectory = rows(load("signflip_across_scale_160m_410m_1b.json")["pythia-160m"]["by_step"])
    loss_rows = sorted((int(k.split("_", 1)[1]), float(v["released_pythia_160m_loss"]))
                       for k, v in loss["by_step"].items())
    ...
    accuracy_value = 100 * np.asarray([float(v["ioi_acc"]) for _, v in trajectory])
    ...
    axes[0].set(ylabel="Pile eval-sample loss", title="Loss decreases across sampled checkpoints")
    axes[1].plot(accuracy_step, accuracy_value, marker="o", linewidth=1.3, markersize=3)
    axes[1].axhline(50, linestyle="--", linewidth=0.8, color="0.55")
    axes[1].set(xscale="log", ylim=(20, 105), xlabel="Training step", ylabel="IOI accuracy (%)")
